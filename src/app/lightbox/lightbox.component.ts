...
export class LightboxComponent {
  ...
  */
  ...
  /**
  * Called when the lightbox image loads. Triggers the fade-in effect only if
  * the loaded image is the one currently selected in the service.
  * @param {string} src The source URL of the image that has loaded.
  * @returns {void}
  */
  public onImageLoad(src: string): void {
    if (src === this.lightboxService.currentImage()?.src) {
      this.loaded = true;
    }
  }
